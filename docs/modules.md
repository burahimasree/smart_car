...
- `porcupine_runner.py`: Porcupine wakeword using single .ppn model (`picovoice_en_raspberry-pi_v3_0_0.ppn`) with access key from `PV_ACCESS_KEY`. Publishes `ww.detected` payload `{timestamp, keyword:"genny", variant:"genny", confidence, source}`. Simulation via `--sim`.
